...
## Regression coverage for the complete Bajie Origin Route.
const CHAPTER_IDS := ["BAJIE-01", "BAJIE-02", "BAJIE-03", "BAJIE-04", "BAJIE-05", "BAJIE-06", "BAJIE-07", "BAJIE-08", "BAJIE-09"]
const CHOICE_IDS := {"BAJIE-02": "HOLD_BACK", "BAJIE-06": "CHOOSE_HOME"}
...
static func run_all() -> Dictionary:
	...
	for chapter_id in CHAPTER_IDS:
		var current := manager.origin_routes.get_current_chapter(manager, "BAJIE")
		...
		if not has_battle:
			var completed := manager.complete_origin_chapter("BAJIE")
		...
		if chapter_id == "BAJIE-06":
			assert(manager.save(), "Bajie progress should save after BAJIE-06")
			var loaded := NarrativeManager.new()
			assert(loaded.load(), "Bajie progress should reload")
			assert(loaded.state.starting_character == "BAJIE", "reload should preserve Bajie route")
			assert(loaded.state.current_origin_chapter == "BAJIE-07", "reload should resume at BAJIE-07")
			manager = loaded

	assert(choices_verified == 2, "Bajie should verify two choices")
	assert(battles_verified == 2, "Bajie should verify two battles")
	assert(manager.origin_routes.is_complete(manager, "BAJIE"), "Bajie route should complete")
	assert(manager.state.route_progress.get("BAJIE", "") == NarrativeState.ROUTE_COMPLETE, "Bajie route should be marked complete")
	assert(manager.state.current_origin_chapter == "", "completed Bajie route should clear active origin chapter")
	assert(manager.state.origin_choices.get("BAJIE-02", "") == "HOLD_BACK", "BAJIE-02 choice should persist")
	assert(manager.state.origin_choices.get("BAJIE-06", "") == "CHOOSE_HOME", "BAJIE-06 choice should persist")
	...
	return {
		...
		"save_reload_checkpoint": "BAJIE-06",
		"route_complete": true,
	}
