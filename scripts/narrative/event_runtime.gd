...
## The manager is a Variant on purpose: this avoids a global-class dependency cycle
## between NarrativeManager, event managers and EventRuntime.

static func from_data(data: Dictionary) -> EventDefinition:
	return EventDefinition.new(data)

static func can_present(event: EventDefinition, manager: Variant, namespace: String) -> bool:
...
static func selected_choice(event: EventDefinition, manager: Variant, namespace: String) -> String:
	if event == null or manager == null or not _valid_namespace(namespace):
		return ""
	return _selected_choice(event, manager, namespace)

static func apply_choice(event: EventDefinition, manager: Variant, choice_id: String, namespace: String) -> bool:
...
static func _selected_choice(event: EventDefinition, manager: Variant, namespace: String) -> String:
	if event.get_id() == "":
		return ""
	if namespace == "ORIGIN":
...
static func _apply_effects(manager: Variant, effects_value: Variant) -> void:
