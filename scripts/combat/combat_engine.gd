...
func perform_action(actor: Combatant, target: Combatant, action: CombatAction, boosted: bool = false) -> Dictionary:
	...
	var effect_result := _apply_action_effects(actor, target, action)
...
func _apply_action_effects(actor: Combatant, target: Combatant, action: CombatAction) -> Dictionary:
	...
	match effect:
		"slow":
			...
			combat_log.emit("%s slows %s: SPD %+d for %d turns." % [actor.display_name, target.display_name, value, duration])
			return {"effect": effect, "duration": duration}
		"taunt":
			target.aggro_turns = maxi(target.aggro_turns, duration)
			combat_log.emit("%s forces %s to focus attacks for %d turns." % [actor.display_name, target.display_name, duration])
			return {"effect": effect, "duration": duration}
		"shield":
			var gained := target.gain_barrier(value)
			combat_log.emit("%s gains %d barrier (total %d)." % [target.display_name, value, gained])
			return {"effect": effect, "duration": duration}
	return {}
