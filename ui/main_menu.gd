...
const ROUTE_LENGTHS := {"TANG":"8章", "WUKONG":"15章", "BAJIE":"9章", "WUJING":"8章", "LONGMA":"6章"}
...
func _panel(parent: Node, min_size := Vector2.ZERO) -> PixelFrame:
	var panel := PixelFrame.new()
...
func _build_ui() -> void:
	...
	contact_button.tooltip_text = CONTACT_WECHAT
	contact_button.custom_minimum_size = Vector2(0, 36)
	contact_button.pressed.connect(_show_contact)
	left_box.add_child(contact_button)
	...
	right_box.add_child(_label("人物序章", 14, Color("a99569")))
	...
	var footer_dev := _label("开发者：%s" % DEVELOPER_NAME, 12, Color("8f866f"))
	footer.add_child(footer_dev)
	var footer_right := _label("Journey to the West", 12, Color("8f866f"))
...
func _refresh_ui() -> void:
	...
	if narrative.state.starting_character == "":
		...
		current_label.text = "当前存档\n起始主角：%s\n世界时间：T%04d\n共享章节：%s\n已招募：%s" % [CHARACTER_NAMES.get(narrative.state.starting_character, narrative.state.starting_character), narrative.state.current_global_timeline, narrative.state.current_shared_chapter if narrative.state.current_shared_chapter != "" else "尚未汇合", _join_character_names(narrative.state.recruited_characters)]
...
func _create_contact_dialog() -> AcceptDialog:
	...
	dialog.dialog_text = "开发者：%s\n微信：%s\n\n欢迎讨论游戏合作、发行与投资。" % [DEVELOPER_NAME, CONTACT_WECHAT]
	dialog.ok_button_text = "复制微信号"
	dialog.confirmed.connect(func(): DisplayServer.clipboard_set(CONTACT_WECHAT))
	return dialog
