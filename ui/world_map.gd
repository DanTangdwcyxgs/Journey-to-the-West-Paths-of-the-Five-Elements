...
func _build_ui() -> void:
	...
	var background := ColorRect.new(); background.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); background.color = Color("0b0d12"); add_child(background)
	var margin := MarginContainer.new(); margin.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); margin.add_theme_constant_override("margin_left",40); margin.add_theme_constant_override("margin_right",40); margin.add_theme_constant_override("margin_top",30); margin.add_theme_constant_override("margin_bottom",30); add_child(margin)
	var root := VBoxContainer.new(); root.add_theme_constant_override("separation",12); margin.add_child(root)
	var title := Label.new(); title.text = "西游地图 · 行路与传闻"; title.add_theme_font_size_override("font_size",30); root.add_child(title)
	status_label = Label.new(); status_label.add_theme_font_size_override("font_size",16); root.add_child(status_label)
	...
	var left := VBoxContainer.new(); left.custom_minimum_size = Vector2(360,0); body.add_child(left)
	var node_title := Label.new(); node_title.text = "可到达地点"; node_title.add_theme_font_size_override("font_size",18); left.add_child(node_title)
	node_list = ItemList.new(); node_list.size_flags_vertical = Control.SIZE_EXPAND_FILL; node_list.item_selected.connect(_on_node_selected); left.add_child(node_list)
	travel_button = Button.new(); travel_button.text = "前往地点"; travel_button.custom_minimum_size = Vector2(0,48); travel_button.pressed.connect(_travel); left.add_child(travel_button)
	...
	detail_label = Label.new(); detail_label.custom_minimum_size = Vector2(0,200); detail_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; detail_label.add_theme_font_size_override("font_size",17); center.add_child(detail_label)
	bounty_label = Label.new(); bounty_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; center.add_child(bounty_label)
	bounty_button = Button.new(); bounty_button.text = "接受悬赏并出战"; bounty_button.custom_minimum_size = Vector2(0,46); bounty_button.pressed.connect(_challenge_bounty); center.add_child(bounty_button)
	var rumor_title := Label.new(); rumor_title.text = "当前位置的新传闻"; rumor_title.add_theme_font_size_override("font_size",18); center.add_child(rumor_title)
	rumor_list = ItemList.new(); rumor_list.custom_minimum_size = Vector2(0,150); center.add_child(rumor_list)
	rumor_button = Button.new(); rumor_button.text = "听取选中传闻"; rumor_button.custom_minimum_size = Vector2(0,44); rumor_button.pressed.connect(_hear_rumor); center.add_child(rumor_button)
	var footer := HBoxContainer.new(); footer.add_theme_constant_override("separation",8); root.add_child(footer)
...
func _rest_at_camp() -> void:
	...
	if result.get("ok", false): status_label.text = "已在 %s 休整，队伍状态恢复完成。" % str(world.get_node(world.get_current_location(narrative)).get("name", "营地"))
	else: status_label.text = "暂时无法休整。"
