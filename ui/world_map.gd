...
func _build_ui() -> void:
	...
	party.text = "队伍编成"
	party.pressed.connect(func(): get_tree().change_scene_to_file("res://ui/party_screen.tscn"))
	footer.add_child(party)
	var journey := Button.new()
	journey.text = "返回行程"
	journey.pressed.connect(func(): get_tree().change_scene_to_file("res://ui/journey.tscn"))
	footer.add_child(journey)
	var menu := Button.new()
	menu.text = "主菜单"
	menu.pressed.connect(func(): get_tree().change_scene_to_file("res://ui/main_menu.tscn"))
...
func _refresh() -> void:
	...
	status_label.text = "T%04d · 当前：%s · 队伍 %d/5 · 已探索 %d处 · 已知悬赏 %d" % [
		narrative.state.current_global_timeline,
		str(current_node.get("name", "未定")),
		narrative.state.recruited_characters.size(),
		world_state.get("visited_nodes", []).size(),
		world_state.get("discovered_bounties", []).size(),
	]

	node_list.clear()
	for node in world.get_reachable_nodes(narrative):
		var node_id := str(node.get("id", ""))
		var marker := "★" if node_id == current else "→"
		var visited := "· 已到" if node_id in world_state.get("visited_nodes", []) else "· 未到"
		node_list.add_item("%s %s  T%03d  %s" % [marker, str(node.get("name", node_id)), int(node.get("timeline", 0)), visited])
		if node_id == selected_node_id:
			node_list.select(node_list.item_count - 1)

	if selected_node_id.is_empty():
		selected_node_id = current
	if not selected_node_id.is_empty():
		_render_node(world.get_node(selected_node_id))
...
func _render_node(node: Dictionary) -> void:
	...
	var node_id := str(node.get("id", ""))
	var bounties:Array = node.get("bounties", [])
	detail_label.text = "%s\n\n%s\n\n节点时间：T%03d\n连接：%s" % [
		str(node.get("name", node_id)),
		str(node.get("description", "")),
		int(node.get("timeline", 0)),
...
func _refresh_rumors() -> void:
	...
	for rumor in rumors:
		rumor_list.add_item("%s：%s" % [str(rumor.get("speaker", "路人")), str(rumor.get("text", ""))])
	rumor_button.disabled = rumor_list.item_count == 0
	if rumor_list.item_count == 0:
		rumor_list.add_item("暂无新的传闻。探索新的地点，或推进西游时间线。")

func _on_node_selected(index: int) -> void:
	var reachable := world.get_reachable_nodes(narrative)
	if index < 0 or index >= reachable.size():
		return
	selected_node_id = str(reachable[index].get("id", ""))
	_render_node(reachable[index])
	travel_button.disabled = selected_node_id == world.get_current_location(narrative)

func _travel() -> void:
	if selected_node_id.is_empty():
		return
	if world.visit(narrative, selected_node_id):
		narrative.save()
		_refresh()
		status_label.text += " · 已抵达"
