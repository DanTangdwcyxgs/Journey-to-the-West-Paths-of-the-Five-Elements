...
func _process_scene() -> void:
	...
	if control != null:
		control.theme = PixelUI.build_theme()
	queue_redraw()
...
func _cut_rect(rect: Rect2, cut := 12.0) -> PackedVector2Array:
	...
	var c: float = minf(cut, minf(w, h) * 0.22)
...
func _frame(rect: Rect2, accent := CYAN, cut := 12.0, fill_alpha := 0.16) -> void:
	if rect.size.x < 20.0 or rect.size.y < 14.0:
		return
	var outer: PackedVector2Array = _cut_rect(rect, cut)
	var inner: PackedVector2Array = _cut_rect(rect.grow(-4.0), maxf(6.0, cut - 3.0))
	...
	for px in range(int(rect.position.x + cut + 9), int(maxf(rect.position.x + cut + 9, rect.end.x - cut - 6)), 11):
		draw_rect(Rect2(float(px), rect.position.y + 5, 3, 2), Color(accent, 0.24))
...
func _bar(rect: Rect2, ratio: float, accent := CYAN, segments := 16) -> void:
	var safe: float = clampf(ratio, 0.0, 1.0)
	var gap: float = 2.0
	var width: float = maxf(1.0, (rect.size.x - gap * float(segments - 1)) / float(segments))
	for i in range(segments):
		var x: float = rect.position.x + float(i) * (width + gap)
		var filled: bool = float(i) / float(segments) < safe
...
func _draw_main_menu() -> void:
	...
	for i in range(9):
		var x: float = size.x * 0.38 + float(i) * 43.0
...
func _draw_turn_strip(root: BattleUI) -> void:
	_text(Vector2(32, 45), "TURN //", 12, CYAN)
	var actors: Array = []
	actors.append_array(root.allies)
	actors.append_array(root.enemies)
	var max_count: int = min(7, actors.size())
	var start_x: float = 122.0
	for i in range(max_count):
		var x: float = start_x + float(i) * 82.0
		var live: bool = actors[i].is_alive()
...
func _draw_target_box(root: BattleUI) -> void:
	var target_name: String = "TARGET // UNKNOWN"
	var hp_ratio: float = 1.0
	var shield_ratio: float = 0.0
	if root.selected_target != null:
...
func _draw_team_box(root: BattleUI) -> void:
	var actor_name: String = "WUKONG"
	var hp_ratio: float = 1.0
	if root.selected_ally != null:
...
func _draw_world_map() -> void:
	...
	for i in range(10):
		var x: float = 435.0 + float(i) * 55.0
